...
class CourseDetailsBloc extends Bloc<CourseDetailsEvent, CourseDetailsState> {
  ...
  CourseDetailsBloc(this._getSingleCoursesUsecase, this._toastService)
    ...
    on<GetCourseLesson>(_handleGetCourseLesson);


    on<ChangeVideoUrlEvent>(_handlechangeVideoUrl);


    on<ChangeVideoUrlOfCourseLessonEvent>(_handleChangeVideoUrlOfCourseLesson);
  ...
  FutureOr<void> _handleGetCourseLesson(

      GetCourseLesson event, Emitter<CourseDetailsState> emit) async {
    ...
    final CourseLessonsResponseModel courseLessonsResponseModel =
        ...
        await getCourseLesson(courseId: event.courseId);


    emit(CourseLessonLoadedState(

        courseLessonsResponseModel.data?.first.lessons?.first.lessonVideoUrl ??
  ...
  FutureOr<void> _handleChangeVideoUrlOfCourseLesson(

      ChangeVideoUrlOfCourseLessonEvent event,

      Emitter<CourseDetailsState> emit) {

    CourseLessonLoadedState currentState = state as CourseLessonLoadedState;


    emit(CourseLessonLoadedState(

        event.videoUrl, currentState.courseLessonsResponseModel));
  ...
  Future<CourseLessonsResponseModel> getCourseLesson(

      {required String courseId}) async {

    var result = await _getSingleCoursesUsecase.getCourseLesson(courseId);
    ...
    return result.fold(
      ...
      (CourseLessonsResponseModel data) => data,
